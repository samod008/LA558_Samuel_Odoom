...
#Load shapefile for Des Moines Neighborhood boundaries
DeMoinNeighborhood <- st_read("Neighborhoods.shp")

#Set the coordinate reference system
DeMoinNeighborhood <- st_transform(DeMoinNeighborhood, crs = 4326)

#Plot the shapefile for Des Moines Neighborhood boundaries
m <- leaflet() %>%
  setView(-94.5, 42.2, 6)  %>%
  addTiles() %>%
...
#Use field for outlets number to plot map
m <- leaflet() %>%
  setView(-94.5, 42.2, 6)  %>%
  addTiles() %>%
...
#Plot neighborhood boundaries and points for alcohol stores
m <- leaflet() %>%
  setView(-94.5, 42.2, 6)  %>%
  addTiles() %>%
  addCircles(data = df_sf) %>%
